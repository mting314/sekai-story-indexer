...
* **Quick-action intent detection paraphrase & semantic matching (DONE).** The
  scoped intents in `webapp/server.py` and `src/sekai_story_indexer/query/intent.py`
  are now powered by a **Hybrid Semantic Intent Classifier** combining fast-path
  regexes with **Character N-Gram TF-IDF Exemplar Cosine Similarity**. Matches natural
  paraphrases ("how did everything turn out?", "what's the outcome?", "where do they
  end up?") and multilingual queries (EN / JP / ZH) keylessly with $0 API cost,
  <0.5ms latency, and zero added dependencies.
