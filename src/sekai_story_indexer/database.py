...
def get_chat_model_name() -> str:
    return os.getenv("SEKAI_CHAT_MODEL", DEFAULT_CHAT_MODEL)


def get_generation_provider_name() -> str:
    return os.getenv("SEKAI_INGEST_PROVIDER", DEFAULT_GENERATION_PROVIDER).strip().lower()


def get_generation_model_name() -> str:
    return os.getenv("SEKAI_INGEST_MODEL") or get_chat_model_name()


def get_router_model_name() -> str:
    if router_model := os.getenv("SEKAI_ROUTER_MODEL"):
        return router_model
...
def get_embedding_model_name() -> str:
    return os.getenv("SEKAI_EMBEDDING_MODEL", DEFAULT_EMBEDDING_MODEL)


def get_chroma_db_path() -> str:
    return os.getenv("SEKAI_CHROMA_DB_PATH", DEFAULT_CHROMA_DB_PATH)
...
def initialize_generation_settings() -> None:
    ...
    if provider == "google":
        ...
        get_openai_api_key()
        if not os.getenv("SEKAI_INGEST_MODEL") and not os.getenv("SEKAI_CHAT_MODEL"):
            raise ValueError(
                "SEKAI_INGEST_PROVIDER=openai requires SEKAI_INGEST_MODEL, "
                "or SEKAI_CHAT_MODEL set to an OpenAI model."
            )
    else:
        raise ValueError(
            f"Unsupported SEKAI_INGEST_PROVIDER {provider!r}. "
...
def create_agentic_generation_model(model_name: str | None = None) -> Any:
    ...
    agentic path uses the Responses API. Set SEKAI_OPENAI_API=chat to force
    ...
    if provider == "google":
        return create_google_model(model_name)
    if provider == "openai":
        if os.getenv("SEKAI_OPENAI_API", "").strip().lower() == "chat":
            return create_openai_model(model_name)
