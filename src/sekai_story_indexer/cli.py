...
@app.command()
def fetch(
    story_root: Path = typer.Option(Path("story"), help="Directory to write the story tree into"),
    limit: int = typer.Option(0, help="Only fetch the N earliest events (0 = all)"),
    event_id: list[int] = typer.Option(None, help="Specific event id(s) to fetch"),
):
    """Download Sekai event stories from the master DB + asset CDN and write the
    canonical story tree, story_order.yaml, and events_index.json.

    Source (same as the autosub fetch_event.py): sekai-world master-db-diff +
    storage.sekai.best CDN. Run where egress to those hosts is allowed.
    """
    ...
    plans = fetch_and_write(
        ...
        log=lambda m: console.print(f"[cyan]{m}[/cyan]"),
    )
